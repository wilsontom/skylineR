...
_Skyline is a Windows OS only application_. The specifications of the Windows Virtual Machine used to develop this package are;
...
The transition information should be a `data.frame` containing the following columns;
  ...
  - name : Transiton name
  - rt : Retention time
  - parent : _m/z_ value of the transitions parent
  - product : _m/z_ value of the transition product
...
`SkylineRunner.exe` is the command line executable which allows `Skyline` to be run through the command line instead of a GUI. The `SkylineRunner.exe` can be downloaded from [here](https://skyline.ms/wiki/home/software/Skyline/download.view?entityId=c33e5f41-9c2a-102f-a8bb-da20258202b3&name=SkylineRunner.exe). 
